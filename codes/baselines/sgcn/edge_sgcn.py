...
from codes.baselines.sgcn.inits import *
from codes.net.base_net import Net


class EdgeSGConv(MessagePassing):
    def __init__(self,
                 in_channels: int,
                 out_channels: int,
                 edge_dim: int,
                 K: int = 1,
                 bias: bool = True):
        super(EdgeSGConv, self).__init__(aggr='add')
...
class SgcnEncoder(Net):
    ...
    def __init__(self,model_config, shared_embeddings=None):
        super(SgcnEncoder, self).__init__(model_config)
...
class SgcnDecoder(Net):
    """
    Compute the graph state with the query
    """
    def __init__(self, model_config):
        super(SgcnDecoder, self).__init__(model_config)
